...
public class SerialiseDeserialiseBT {
	...
	public static String serialize(TreeNode root) {
		if (root == null)
			return "[null]";
		// do level wise accumulation
		List<List<TreeNode>> levelWiseNodesList = new ArrayList<List<TreeNode>>();
		List<TreeNode> currLevelNodes = new LinkedList<>();
		currLevelNodes.add(root);
		while (hasNotNullElement(currLevelNodes)) {
			levelWiseNodesList.add(currLevelNodes);
			List<TreeNode> nextlevelNodes = new LinkedList<>();
			for (TreeNode treeNode : currLevelNodes) {
				if (treeNode != null) {
		...
		for (List<TreeNode> level : levelWiseNodesList) {
	...
	private static boolean hasNotNullElement(List<TreeNode> currLevelNodes) {
		for (TreeNode treeNode : currLevelNodes) {
			if (treeNode != null)
				return true;
		}
	...
	public static TreeNode deserialize(String data) {
		...
		if (split.length == 0 || "null".equals(split[0].trim()) || "".equals(split[0].trim()))
			return null;
		Integer[] treeArr = new Integer[split.length];
		for (int i = 0; i < treeArr.length; i++) {
			if (split[i].trim().equals("null"))
				treeArr[i] = null;
			else
				treeArr[i] = Integer.valueOf(split[i].trim());
		}

		Queue<TreeNode> treeQ = new LinkedList<>();
		TreeNode root = new TreeNode(treeArr[0]);
		if (treeArr.length == 2) {
			root.left = treeArr[1] == null ? null : new TreeNode(treeArr[1]);
			return root;
		}
		...
		for (int i = 0; i < treeArr.length / 2; i++) {
			...
			if (curr == null) {
				...
				curr.left = treeArr[i * 2 + 1] == null ? null : new TreeNode(treeArr[i * 2 + 1]);
				curr.right = treeArr[i * 2 + 2] == null ? null : new TreeNode(treeArr[i * 2 + 2]);
				treeQ.add(curr.left);
